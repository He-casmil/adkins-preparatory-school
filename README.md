# Adkins Preparatory School Website

This workspace contains a responsive school website for Adkins Preparatory School.

## Files
- `index.html` — Home page
- `about.html` — About page
- `academics.html` — Academics page
- `admission.html` — Admission page
- `news-events.html` — News & Events page
- `contact.html` — Contact page
- `apply-now.html` — Apply Now page
- `styles.css` — Shared stylesheet
- `scripts.js` — Mobile menu toggle

## Deploy Online
You can publish this site with GitHub Pages or any static site host.

### GitHub Pages
1. Create a GitHub repository and push these files.
2. In repository settings, enable GitHub Pages from the `main` branch.
3. Your site will be available at `https://<username>.github.io/<repository-name>/`.

### Netlify
1. Create a Netlify account.
2. Connect your GitHub repository.
3. Deploy the site as a static project.

### Local Preview
Open `index.html` in your browser or use a local web server.
